...
@onready var card_arc: Line2D = $CanvasLayer/MoveArc
...
func _ready() -> void:
	Events.move_aim_started.connect(_on_card_aim_started)
	Events.move_aim_ended.connect(_on_card_aim_ended)
...
func _on_card_aim_started(card: CardUI) -> void:
	if not card.move.is_single_targeted():
		return
